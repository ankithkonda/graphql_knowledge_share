...
export const handler = async (
    ...
    try {
        response = await axios.get<RESTAPIProjectResponse>(`${process.env.HOST}/api/projects/${event.arguments.id}`, {
            headers: {
    ...
    // try {
    //     logoResponse = await axios.get(`${process.env.HOST}${data.Links.Logo}`, {
    //         headers: {
    //             'X-Octopus-ApiKey': process.env.API_KEY,
    //         },
    //     });
    ...
    const logoSVGString = 'path/to/logo.png'; //logoResponse.data; // commented out as request for base64 takes too long

    let variablesResponse = null;

    try {
        variablesResponse = await axios.get<RESTAPIVariables>(`${process.env.HOST}${data.Links.Variables}`, {
            headers: {
                'X-Octopus-ApiKey': process.env.API_KEY,
            },
        });
